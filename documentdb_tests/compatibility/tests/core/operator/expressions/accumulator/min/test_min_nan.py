...
MIN_NAN_TESTS: list[ExpressionTestCase] = [
    ...
    ExpressionTestCase(
        ...
        msg="$min should return NaN when float NaN is the sole operand",
    ),
    ExpressionTestCase(
        "nan_decimal128_sole",
        expression={"$min": ["$a"]},
        ...
        msg="$min should return Decimal128 NaN when it is the sole operand",
    ),
    ...
    # NaN alongside numeric values: NaN is the minimum.
    ExpressionTestCase(
        "nan_float_with_ints",
        expression={"$min": ["$a", "$b", "$c", "$d"]},
        doc={"a": 1, "b": 2, "c": 3, "d": FLOAT_NAN},
        expected=pytest.approx(math.nan, nan_ok=True),
        msg="$min should return float NaN when it is among integers",
    ),
    ExpressionTestCase(
        ...
        expected=DECIMAL128_NAN,
        msg="$min should return Decimal128 NaN over non-NaN value",
    ),
    ExpressionTestCase(
    ...
    # NaN is less than -Infinity, so $min picks NaN.
    ExpressionTestCase(
        "nan_float_vs_neg_inf",
        expression={"$min": ["$a", "$b"]},
        doc={"a": FLOAT_NAN, "b": FLOAT_NEGATIVE_INFINITY},
        expected=pytest.approx(math.nan, nan_ok=True),
        msg="$min should pick float NaN over -Infinity",
    ),
    ExpressionTestCase(
        ...
        expected=pytest.approx(math.nan, nan_ok=True),
        msg="$min should pick float NaN over -Infinity regardless of order",
    ),
    ExpressionTestCase(
        ...
        expected=pytest.approx(math.nan, nan_ok=True),
        msg="$min should pick float NaN over Decimal128 -Infinity",
    ),
    ExpressionTestCase(
        ...
        expected=DECIMAL128_NAN,
        msg="$min should pick Decimal128 NaN over float -Infinity",
    ),
    ExpressionTestCase(
        ...
        expected=DECIMAL128_NAN,
        msg="$min should pick Decimal128 NaN over Decimal128 -Infinity",
    ),
    # NaN is less than negative numbers, so $min picks NaN.
    ExpressionTestCase(
        "nan_float_vs_negative",
        expression={"$min": ["$a", "$b"]},
        doc={"a": FLOAT_NAN, "b": -1000},
        expected=pytest.approx(math.nan, nan_ok=True),
        msg="$min should pick float NaN over negative number",
    ),
    ExpressionTestCase(
        ...
        expected=DECIMAL128_NAN,
        msg="$min should pick Decimal128 NaN over negative number",
    ),
    # NaN is numeric type > MinKey in BSON order, so $min picks MinKey.
    ExpressionTestCase(
        "nan_float_vs_minkey",
        expression={"$min": ["$a", "$b"]},
        doc={"a": FLOAT_NAN, "b": MinKey()},
        expected=MinKey(),
        msg="$min should pick MinKey over float NaN per BSON type order",
    ),
    ExpressionTestCase(
        "nan_minkey_vs_float",
        expression={"$min": ["$a", "$b"]},
        ...
        expected=MinKey(),
        msg="$min should pick MinKey over float NaN regardless of order",
    ),
    ExpressionTestCase(
        ...
        expected=MinKey(),
        msg="$min should pick MinKey over Decimal128 NaN per BSON type order",
    ),
    ...
    ExpressionTestCase(
        ...
        expected=pytest.approx(math.nan, nan_ok=True),
        msg="$min should return float NaN when it is the only non-null value",
    ),
    ExpressionTestCase(
        ...
        expected=DECIMAL128_NAN,
        msg="$min should return Decimal128 NaN when it is the only non-null value",
    ),
    ...
    ExpressionTestCase(
        ...
        expected=pytest.approx(math.nan, nan_ok=True),
        msg="$min should return float NaN when it appears before Decimal128 NaN",
    ),
    ExpressionTestCase(
        ...
        expected=DECIMAL128_NAN,
        msg="$min should return Decimal128 NaN when it appears before float NaN",
    ),
]


@pytest.mark.parametrize("test_case", pytest_params(MIN_NAN_TESTS))
def test_min_nan_cases(collection, test_case: ExpressionTestCase):
    """Test $min NaN behavior cases."""
