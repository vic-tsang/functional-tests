...
OBJECT_TESTS: list[ExpressionTestCase] = [
    ...
    ExpressionTestCase(
        "empty_obj_self", expression={"$lte": [{}, {}]}, expected=True, msg="{} <= {} (self)"
    ),
]
...
ARRAY_TESTS: list[ExpressionTestCase] = [
    ...
    ExpressionTestCase(
        "arr_empty_lte_empty", expression={"$lte": [[], []]}, expected=True, msg="[] <= []"
    ),
    ExpressionTestCase(
        "arr_2_lte_1", expression={"$lte": [[2], [1]]}, expected=False, msg="[2] not <= [1]"
    ),
    ExpressionTestCase(
        "arr_1_lte_2", expression={"$lte": [[1], [2]]}, expected=True, msg="[1] <= [2]"
    ),
    ExpressionTestCase(
        "arr_1_self", expression={"$lte": [[1], [1]]}, expected=True, msg="[1] <= [1]"
    ),
    ExpressionTestCase(
    ...
    ExpressionTestCase(
        "empty_arr_self", expression={"$lte": [[], []]}, expected=True, msg="[] <= [] (self)"
    ),
    ExpressionTestCase(
        "empty_lte_arr_1", expression={"$lte": [[], [1]]}, expected=True, msg="[] <= [1]"
    ),
    ExpressionTestCase(
        "arr_null_lte_empty",
        expression={"$lte": [[None], []]},
