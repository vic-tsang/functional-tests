"""
Tests for $gte argument handling.
...
GTE_ARG_TESTS: list[ExpressionTestCase] = [
    ExpressionTestCase(
        "no_args",
        expression={"$gte": []},
    ...
    ExpressionTestCase(
        "non_array_string",
        expression={"$gte": "string"},
        error_code=EXPRESSION_TYPE_MISMATCH_ERROR,
        msg="Should error for string argument",
    ),
    ExpressionTestCase(
        "three_args",
        expression={"$gte": [3, 2, 1]},
        error_code=EXPRESSION_TYPE_MISMATCH_ERROR,
...
@pytest.mark.parametrize("test", pytest_params(GTE_ARG_TESTS))
def test_gte_argument_handling(collection, test):
    """Test $gte argument count variations."""
