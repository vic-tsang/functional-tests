"""
Tests for $gt expression type smoke tests.
...
LITERAL_TESTS: list[ExpressionTestCase] = [
    ExpressionTestCase(
        "literal_true", expression={"$gt": [5, 3]}, expected=True, msg="Literal 5 > 3"
    ),
    ExpressionTestCase(
        "literal_false",
        expression={"$gt": ["hello", "world"]},
        expected=False,
        msg="hello not > world",
    ),
]
...
def test_gt_expression_types_literal(collection, test):
    """Test $gt with literal expression inputs."""
...
def test_gt_expression_types_insert(collection, test):
    """Test $gt with field reference and expression inputs requiring documents."""
