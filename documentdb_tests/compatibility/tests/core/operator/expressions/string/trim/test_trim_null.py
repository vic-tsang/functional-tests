...
def _build_null_tests(null_value, prefix) -> list[TrimTest]:
    return [
        TrimTest(
            f"{prefix}_{suffix}",
            ...
            expected=None,
            msg=f"$trim should return null when {msg_tmpl.format(kind=prefix)}",
        )
...
TRIM_MIXED_NULL_TESTS: list[TrimTest] = [
    ...
    TrimTest(
        ...
        chars=None,
        expected=None,
        msg="$trim should return null when input is missing and chars is null",
    ),
]

TRIM_NULL_ALL_TESTS = TRIM_NULL_TESTS + TRIM_MISSING_TESTS + TRIM_MIXED_NULL_TESTS


@pytest.mark.parametrize("test_case", pytest_params(TRIM_NULL_ALL_TESTS))
def test_trim_null(collection, test_case: TrimTest):
    """Test $trim null propagation."""
