...
ALLelementstrue_CORE_TESTS: list[AllElementsTrueTest] = [
...
@pytest.mark.parametrize("test", pytest_params(ALLelementstrue_CORE_TESTS))
...
# ---------------------------------------------------------------------------
# Falsy position testing — correctness regardless of position
# ---------------------------------------------------------------------------
def test_allElementsTrue_falsy_at_start(collection):
    """Test $allElementsTrue with falsy at index 0 of 100-element array."""
    result = execute_expression(collection, {"$allElementsTrue": [[False] + [True] * 99]})
    assert_expression_result(
        result, expected=False, msg="Should return false when falsy element is at start"
    )


def test_allElementsTrue_falsy_at_middle(collection):
    """Test $allElementsTrue with falsy at index 50 of 100-element array."""
    result = execute_expression(
        collection, {"$allElementsTrue": [[True] * 50 + [False] + [True] * 49]}
    )
    assert_expression_result(
        result, expected=False, msg="Should return false when falsy element is at middle"
    )


def test_allElementsTrue_falsy_at_end(collection):
    """Test $allElementsTrue with falsy at index 99 of 100-element array."""
    result = execute_expression(collection, {"$allElementsTrue": [[True] * 99 + [False]]})
    assert_expression_result(
        result, expected=False, msg="Should return false when falsy element is at end"
    )


def test_allElementsTrue_no_falsy_100(collection):
    """Test $allElementsTrue with 100-element all-truthy array."""
    result = execute_expression(collection, {"$allElementsTrue": [[True] * 100]})
    assert_expression_result(
        result, expected=True, msg="Should return true for 100 truthy elements"
    )


# ---------------------------------------------------------------------------
# Scale testing — 1000+ elements
# ---------------------------------------------------------------------------
def test_allElementsTrue_scale_all_truthy(collection):
    """Test $allElementsTrue with 1000+ element all-truthy array."""
    result = execute_expression(collection, {"$allElementsTrue": [[True] * 1001]})
    assert_expression_result(
        result, expected=True, msg="Should return true for 1001 truthy elements"
    )


def test_allElementsTrue_scale_falsy_last(collection):
    """Test $allElementsTrue with 1000+ elements, last is falsy."""
    result = execute_expression(collection, {"$allElementsTrue": [[True] * 1000 + [False]]})
    assert_expression_result(
        result, expected=False, msg="Should return false when last element is falsy"
    )


def test_allElementsTrue_scale_falsy_first(collection):
    """Test $allElementsTrue with 1000+ elements, first is falsy."""
    result = execute_expression(collection, {"$allElementsTrue": [[False] + [True] * 1000]})
    assert_expression_result(
        result, expected=False, msg="Should return false when first element is falsy"
    )


def test_allElementsTrue_scale_range_contains_zero(collection):
    """Test $allElementsTrue with range(1000) — contains 0."""
