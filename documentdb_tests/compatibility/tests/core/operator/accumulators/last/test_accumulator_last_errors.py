"""Tests for $last accumulator error cases: arity rejection and expression error propagation."""
...
# Property [Arity]: $last in accumulator context is a unary operator and
...
LAST_ARITY_ERROR_TESTS: list[AccumulatorTestCase] = [
    AccumulatorTestCase(
        ...
        docs=[{"v": 1}],
        pipeline=[
            {"$group": {"_id": None, "result": {"$last": []}}},
            {"$project": {"_id": 0, "result": 1}},
        ],
        error_code=GROUP_ACCUMULATOR_ARRAY_ARGUMENT_ERROR,
        msg="$last should reject empty array in accumulator context",
    ),
    AccumulatorTestCase(
        "arity_single_element_array",
        docs=[{"v": 1}],
        pipeline=[
            {"$group": {"_id": None, "result": {"$last": [1]}}},
            {"$project": {"_id": 0, "result": 1}},
        ],
        error_code=GROUP_ACCUMULATOR_ARRAY_ARGUMENT_ERROR,
    ...
    AccumulatorTestCase(
        ...
        docs=[{"v": 1}],
        pipeline=[
            {"$group": {"_id": None, "result": {"$last": [1, 2, 3]}}},
            {"$project": {"_id": 0, "result": 1}},
        ],
        error_code=GROUP_ACCUMULATOR_ARRAY_ARGUMENT_ERROR,
        msg="$last should reject multi-element array in accumulator context",
    ),
    AccumulatorTestCase(
...
LAST_EXPRESSION_ERROR_TESTS: list[AccumulatorTestCase] = [
    AccumulatorTestCase(
    ...
    AccumulatorTestCase(
        ...
        docs=[{"v": "not_a_number"}],
        pipeline=[
            {"$group": {"_id": None, "result": {"$last": {"$toInt": "$v"}}}},
            {"$project": {"_id": 0, "result": 1}},
        ],
        error_code=CONVERSION_FAILURE_ERROR,
        msg="$last should propagate $toInt conversion error",
    ),
]

LAST_ERROR_TESTS = LAST_ARITY_ERROR_TESTS + LAST_EXPRESSION_ERROR_TESTS


@pytest.mark.parametrize("test_case", pytest_params(LAST_ERROR_TESTS))
def test_accumulator_last_errors(collection, test_case):
    """Test $last accumulator error cases."""
    if test_case.docs:
