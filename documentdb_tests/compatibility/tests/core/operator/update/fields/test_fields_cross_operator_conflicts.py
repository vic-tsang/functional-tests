...
CROSS_OPERATOR_CONFLICT_TESTS: list[UpdateTestCase] = [
    UpdateTestCase(
        id="set_setOnInsert_same_path",
        query={"_id": 1},
        update={"$set": {"a": 1}, "$setOnInsert": {"a": 2}},
        upsert=True,
        error_code=CONFLICTING_UPDATE_OPERATORS_ERROR,
        msg="$set and $setOnInsert on same field should fail",
    ),
    UpdateTestCase(
...
CROSS_OPERATOR_SUCCESS_TESTS: list[UpdateTestCase] = [
    ...
    UpdateTestCase(
        id="setOnInsert_and_inc_no_match",
        query={"_id": 1},
        update={"$setOnInsert": {"x": 1}, "$inc": {"count": 1}},
        upsert=True,
        expected=[{"_id": 1, "x": 1, "count": 1}],
        msg="$setOnInsert and $inc both apply on insert",
    ),
    UpdateTestCase(
        id="setOnInsert_and_rename_no_match",
        query={"_id": 1},
        update={"$setOnInsert": {"a": 1}, "$rename": {"b": "c"}},
        upsert=True,
        expected=[{"_id": 1, "a": 1}],
        msg="$setOnInsert applies, $rename is no-op on missing field",
    ),
]
